    ...
    return (
        <div className="max-w-4xl mx-auto px-2 py-4 md:p-10 min-h-screen flex flex-col">
            <div className="flex justify-between items-center mb-6 md:mb-8 border-b border-slate-200 dark:border-slate-700 pb-4">
                <div>
                    <Link href={`/${lang}/biology-small`} className="text-sm text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 -ml-1 mb-3 flex items-center gap-1 font-medium group">
                    ...
                    {!isSubmitted ? (
                        ...
                        <div className="flex gap-4 w-full md:w-auto">
                            {currentQ.explanation && (
                                <button onClick={() => setShowExplanation(!showExplanation)} className={`scale-[0.97] md:scale-100 px-6 py-4 rounded-xl font-bold transition-all border ${showExplanation ? 'bg-amber-100 border-amber-300 text-amber-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{showExplanation ? '隐藏解析' : '查看解析'}</button>
                            )}
                            <button onClick={handleNext} className="scale-[0.97] md:scale-100 flex-grow md:flex-grow-0 px-8 py-4 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 hover:text-indigo-600 transition-all shadow-sm">下一题 <ChevronRight size={20} /></button>
                        </div>
                    )}
                </div>
            </div>
