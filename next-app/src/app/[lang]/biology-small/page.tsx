    ...
    <div className="max-w-4xl mx-auto py-10 px-6">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight">
          生物小题特训 <span className="text-violet-600 dark:text-slate-300">Biology</span>
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {CATEGORIES.map((category) => (
          <Link 
            ...
            className="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm md:hover:shadow-xl md:hover:border-violet-200 md:dark:hover:border-slate-700 transition-all duration-200 md:hover:-translate-y-1 active:scale-95 active:bg-slate-50 dark:active:bg-slate-800 z-10 block"
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 md:group-hover:opacity-10 transition-opacity pointer-events-none">
              <span className="text-6xl">{category.icon}</span>
            </div>
            <div className="flex flex-col h-full">
               <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800/50 rounded-xl flex items-center justify-center text-2xl mb-4 md:group-hover:bg-violet-600 md:group-hover:text-white transition-colors">
                 {category.icon}
               </div>
