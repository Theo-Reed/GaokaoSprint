    ...
    return (
      <div className="max-w-4xl mx-auto py-12 px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500">{isCn ? '全真模拟高考写作环境，配备四模型 AI 联合测评。' : 'Simulate Gaokao writing environment with Multi-AI evaluation.'}</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          <button 
            onClick={() => { setMode('write'); setType('small'); startTraining('small'); }}
            className="group p-8 bg-white border-2 border-slate-200 rounded-2xl hover:border-indigo-400 hover:shadow-xl transition-all text-left"
          >
            ...
            <p className="text-slate-500 text-sm">应用文写作（书信、通知等）。要求 80 词左右。</p>
          </button>
          <button 
          ...
          >
            ...
            <p className="text-slate-500 text-sm">根据给定开头续写故事。要求 150 词左右。</p>
          </button>
        </div>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500">{isCn ? '严格按照高考满分标准，为你生成高分范文。' : 'Generate high-score model essays based on Gaokao standards.'}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            ...
            <p className="text-slate-500 text-sm">提供题目，瞬间生成满分级应用文范文。</p>
          </button>
          <button 
          ...
          >
            ...
            <p className="text-slate-500 text-sm">攻克读后续写，生成情节生动、语言地道的范文。</p>
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex flex-wrap justify-between items-end gap-4 mb-8">
        <div>
          <button 
            ...
            className="text-sm text-slate-500 hover:text-indigo-500 mb-2 flex items-center gap-1 font-medium group"
          >
          ...
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            ...
            >
              <span className="text-slate-500 group-hover:text-indigo-500 font-medium">
                {topic.year} {Array.isArray(topic.region) ? topic.region[0] : topic.region}
              </span>
              <Search size={14} className="text-slate-400 group-hover:text-indigo-400" />
            </button>
          </h1>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full font-mono text-xl font-bold text-slate-700">
            <Clock size={20} className="text-slate-400" />
            {formatTime(timer)}
          </div>
          <button 
            onClick={() => startTraining(type)}
            className="p-2 text-slate-400 hover:text-indigo-500 hover:rotate-180 transition-all duration-500"
          ...
          {mode === 'write' ? (
            <div className="flex-grow flex flex-col relative group">
              <textarea
                ...
                className="flex-grow min-h-[450px] w-full p-6 pb-24 text-lg border-2 border-slate-200 rounded-2xl focus:border-indigo-400 focus:ring-0 transition-all bg-white font-serif resize-none text-slate-900 shadow-sm"
              />
            ...
            <div className="flex-grow min-h-[450px] w-full p-8 border-2 border-slate-200 border-dashed rounded-2xl bg-white flex flex-col items-center justify-center text-center">
               ...
               <p className="text-slate-500 max-w-xs">{isCn ? '点击下方按钮，四核 AI 将根据左侧题目要求，严格遵循高考第五档标准为您同步生成多篇风格迥异的满分范文。' : 'Click the button below to generate Tier 5 model essays following Gaokao standards.'}</p>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center">
            <button 
              ...
              className={`
                ...
                ${isSubmitting || (mode === 'write' && !answer.trim()) ? 'bg-slate-300/70 cursor-not-allowed' : 'bg-indigo-500 hover:bg-indigo-600 hover:shadow-indigo-100 hover:-translate-y-0.5 active:scale-[0.98] blur-none'}
              `}
            >
              {isSubmitting ? (
                <>
        ...
        <div className="mt-12 space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {['gemini', 'gpt', 'deepseek', 'claude'].map((model) => {
               ...
               return (
                 <div key={model} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow min-h-[150px] flex flex-col">
                   <div className="flex items-center justify-between mb-3 text-sm font-bold text-slate-400 uppercase tracking-widest">
                     <span>
                     ...
                     {feedback ? (
                       <div className="animate-in fade-in duration-300 space-y-4">
                         ...
                         {mode === 'write' ? (
                             ...
                             <ul className="space-y-[0.7em]">
                               {['内容覆盖', '语言词汇', '句式多样', '连贯衔接'].map((label, i) => {
                                 ...
                                 return (
                                   <li key={i} className="flex gap-2">
                                     ...
                                     <div className="prose prose-slate prose-sm max-w-none prose-strong:text-indigo-600 prose-strong:font-black">
                                       <div className="flex flex-col">
                                         <strong className="text-indigo-600 mb-[0.7em]">[{label}]</strong>
                               ...
                               {feedback.summary && (
                                 <li className="flex gap-2 mt-4 pt-4 border-t border-slate-100">
                                   <div className="prose prose-slate prose-sm max-w-none prose-strong:text-indigo-600 prose-strong:font-black w-full">
                                     <div className="flex flex-col">
                                       <strong className="text-indigo-600 mb-[0.7em]">[{isCn ? '综合点评' : 'Summary'}]</strong>
                                       <div className="[&_p]:my-0 [&_>*:first-child]:mt-0 [&_>*:last-child]:mb-0 text-slate-700 leading-relaxed">
                           ...
                           <div className="prose prose-slate prose-lg max-w-none pt-2 [&_p]:my-[10px]">
                       ...
                       <div className="flex items-center gap-2 text-slate-300 italic font-normal">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl max-h-[80vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-black text-slate-900">{isCn ? '题库精准匹配' : 'Topic Search'}</h3>
                <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">{type === 'small' ? 'Small Composition' : 'Story Continuation'}</p>
              </div>
              <button onClick={() => setIsSelecting(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
            ...
            <div className="p-6 bg-slate-50">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-white border-2 border-slate-200 rounded-2xl focus:border-indigo-500 focus:outline-none text-lg font-medium shadow-inner text-slate-900 placeholder:text-slate-400"
                  value={searchQuery}
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="bg-slate-900 text-white text-[10px] font-black px-2 py-0.5 rounded uppercase">{t.year}</span>
                        <span className="text-slate-800 font-bold">{Array.isArray(t.region) ? t.region.join(' / ') : t.region}</span>
                      </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 text-center text-xs text-slate-400 font-bold">
