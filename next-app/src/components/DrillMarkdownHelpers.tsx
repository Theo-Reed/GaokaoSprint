...
export const markdownComponents = {
  p: ({children}: {children?: React.ReactNode}) => (
    <div className="mb-4 text-slate-800 leading-relaxed font-serif whitespace-pre-wrap">{children}</div>
  ),
  table: ({children}: {children?: React.ReactNode}) => (
    <div className="table-container shadow-sm my-6 border border-slate-200 rounded-lg overflow-hidden overscroll-contain">
      <table className="min-w-full divide-y divide-slate-200">{children}</table>
    </div>
  ),
  th: ({children}: {children?: React.ReactNode}) => (
    <th className="px-4 py-3 bg-slate-50 text-left text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-r border-slate-200 last:border-r-0">{children}</th>
  ),
  td: ({children}: {children?: React.ReactNode}) => (
    <td className="px-4 py-3 text-sm text-slate-600 border-b border-r border-slate-200 last:border-r-0 bg-white">{children}</td>
  )
};
